...
class SwapDealDB {
  ...
  public function nonSQLError($message) {
    if ($this->throw_exception_on_nonsql_error) {
      $e = new SwapDealDBException($message);
      throw $e;
    }
    
    $error_handler = is_callable($this->nonsql_error_handler) ? $this->nonsql_error_handler : 'SwapDealDB_error_handler';
  ...
  public function sqleval() {
    ...
    return new SwapDealDBEval($text);
  }
  ...
  protected function preparseQueryParams() {
    ...
    // generate list of all SwapDealDB variables in our query, and their position
    ...
    // for each SwapDealDB variable, substitute it with array(type: i, value: 53) or whatever
  ...
  protected function sanitize($value) {
    if (is_object($value)) {
      if ($value instanceof SwapDealDBEval) return $value->text;
  ...
  protected function parseQueryParams() {
    ...
    foreach ($chunkyQuery as $chunk) {
      ...
      else $this->nonSQLError("Badly formatted SQL query: Invalid SwapDealDB param $type");
  ...
  protected function queryHelper() {
    ...
    if (!$sql || $db->error) {
      if ($this->error_handler) {
        $error_handler = is_callable($this->error_handler) ? $this->error_handler : 'SwapDealDB_error_handler';
      ...
      if ($this->throw_exception_on_error) {
        $e = new SwapDealDBException($db->error, $sql);
        throw $e;
      }
    } else if ($this->success_handler) {
      $runtime = sprintf('%f', $runtime * 1000);
      $success_handler = is_callable($this->success_handler) ? $this->success_handler : 'SwapDealDB_debugmode_handler';
...
class SwapDealDBException extends Exception {
  protected $query = '';
...
function SwapDealDB_error_handler($params) {
...
function SwapDealDB_debugmode_handler($params) {
